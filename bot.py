...
CONTROL_URL = os.environ.get("CONTROL_URL", "").rstrip("/")  # http://<IP>:25583
...
def whitelist_remove(name: str) -> str:
    print(f"[whitelist] remove {name}")
    if not CONTROL_URL or not CONTROL_KEY:
        return "⛔ CONTROL_URL или CONTROL_KEY не заданы в env"
    try:
        r = requests.get(
            f"{CONTROL_URL}/api/whitelist/remove",
            params={"name": name},
            headers={"X-Api-Key": CONTROL_KEY},
            timeout=10
        )
        ...
        try:
            ...
            for upd in data.get("result", []):
                ...
                if msg and "text" in msg:
                    ...
                    if chat_id == CHAT_ID:
                        frm    = msg.get("from", {}) or {}
                        sender = frm.get("username") or frm.get("first_name") or "TGUser"
                        text   = msg.get("text", "")
                        ...
                        if m_add:
                            ...
                            result = whitelist_add(player)
                            send_tg(result)
                        elif m_rem:
                            player = m_rem.group(1)
                            result = whitelist_remove(player)
                            send_tg(result)
                        else:
                            with lock:
                                tg_to_mc.append({"player": sender, "message": text})
                            print(f"Queued TG->MC: {sender}: {text}")
                    else:
                        print(f"Skipped chat_id={chat_id}, expected={CHAT_ID}")
